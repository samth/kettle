...
(test-case "space starts the stopwatch"
  (define sw (make-stopwatch))
  (define-values (sw2 cmd) (extract-update-result (update sw (char-key #\space))))
...
(test-case "space again stops it"
  (define sw (make-stopwatch))
  (define-values (sw2 _) (extract-update-result (update sw (char-key #\space))))
  (define-values (sw3 cmd) (extract-update-result (update sw2 (char-key #\space))))
...
(test-case "q returns quit command"
  (define sw (make-stopwatch))
  (define-values (sw2 cmd) (extract-update-result (update sw (char-key #\q))))
