...
(test-case "typing into textinput and pressing enter adds item"
  ...
  (define (type-char t ch)
    (define-values (t2 _) (update t (char-key ch)))
    t2)
  ...
  ;; Press enter to add
  (define-values (t3 cmd) (update t2 (sym-key 'enter)))
...
(test-case "pressing enter with empty input does nothing"
  (define t (make-todo))
  (define-values (t2 cmd) (update t (sym-key 'enter)))
...
(test-case "tab switches from insert to navigate"
  (define t (make-todo))
  (check-equal? (todo-mode t) 'insert)
  (define-values (t2 cmd) (update t (sym-key 'tab)))
  (check-equal? (todo-mode t2) 'navigate)
  (check-false cmd))

(test-case "tab switches from navigate back to insert"
  (define t (make-todo))
  (define-values (t2 _) (update t (sym-key 'tab)))
  (define-values (t3 cmd) (update t2 (sym-key 'tab)))
...
(test-case "d in navigate mode deletes selected item"
  ...
  (define (type-and-enter t str)
    (define t2
      (for/fold ([acc t]) ([ch (in-string str)])
        (define-values (next _) (update acc (char-key ch)))
        next))
    (define-values (t3 _) (update t2 (sym-key 'enter)))
    t3)
  ...
  (define-values (t4 _) (update t3 (sym-key 'tab)))
  ;; Delete the selected item (index 0)
  (define-values (t5 cmd) (update t4 (char-key #\d)))
...
(test-case "C-q quits from any mode"
  (define t (make-todo))
  (define-values (t2 cmd) (update t (char-key #\q #:ctrl #t)))
